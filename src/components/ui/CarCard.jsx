...
export default function CarCard({ car }) {
  console.log(car)
  const { addToCart, isInCart, addToWishlist, removeFromWishlist, isInWishlist } = useShop()
      ...
      >
        <Heart
          ...
          className={`transition-colors ${
            isInWishlist(car.id) ? "text-red-500 fill-red-500" : "text-gray-600 hover:text-red-500"
          }`}
        />
      </button>
      ...
      <div className="flex flex-col lg:flex-row h-full">
        ...
        <div className="lg:w-2/5 relative overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-4">
            <Link to={`/cars/${car.id}`}>
              <button className="bg-white text-gray-900 px-4 py-2 rounded-lg font-medium flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                <Eye size={16} />
                View Details
              </button>
            </Link>
          ...
          <div>
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="text-right ml-4">
                <div className="text-2xl font-bold text-red-600">${car.price?.toLocaleString() || "N/A"}</div>
                <div className="text-sm text-gray-500">¥{((car.price || 0) * 150).toLocaleString()}</div>
              </div>
            </div>
            ...
            {/* Reference */}
            <div className="text-sm text-gray-500 mb-4">Ref: #{car.id?.slice(0, 8) || "N/A"}</div>
          </div>
