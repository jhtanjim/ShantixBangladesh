...
export const useAllCars = (filters: CarFilters = {}) => {
  return useQuery({
...
export const useAllCarsAdmin = (filters: CarFilters = {}) => {
  return useQuery({
...
export const useUpdateCar = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<Car> }) => updateCarById(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cars"] })
      queryClient.invalidateQueries({ queryKey: ["cars-admin"] })
